<?php declare(strict_types=1);

require_once(__DIR__ . "/../../../libs/composer/vendor/autoload.php");


trait ProgressRepoMockNIFT
{
    public function __construct()
    {
    }

    public function readByIds(int $prg_id, int $assignment_id) : ilStudyProgrammeProgress
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readByPrgIdAndAssignmentId(int $prg_id, int $assignment_id)
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readByPrgIdAndUserId(int $prg_id, int $usr_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readByPrgId(int $prg_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readFirstByPrgId(int $prg_id)
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readByAssignmentId(int $assignment_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readExpiredSuccessfull() : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readRiskyToFailInstances() : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readPassedDeadline() : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function delete(ilStudyProgrammeProgress $progress)
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function createFor(
        ilStudyProgrammeSettings $prg,
        ilStudyProgrammeAssignment $ass
    ) : ilStudyProgrammeProgress {
        throw new Exception("Not implemented for testing", 1);
    }
}

trait AssignmentRepoMockNIFT
{
    public function __construct()
    {
    }

    public function createFor(int $prg_id, int $usr_id, int $assigning_usr_id) : ilStudyProgrammeAssignment
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readByUsrId(int $usr_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readByPrgId(int $prg_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readDueToRestart() : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function readDueToManuelRestart(int $days_before_end) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function delete(ilStudyProgrammeAssignment $assignment)
    {
        throw new Exception("Not implemented for testing", 1);
    }
}

trait SettingsRepoMockNIFT
{
    public function __construct()
    {
    }

    public function createFor(int $obj_id) : ilStudyProgrammeSettings
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function delete(ilStudyProgrammeSettings $settings) : void
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function loadByType(int $type_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
    public function loadIdsByType(int $type_id) : array
    {
        throw new Exception("Not implemented for testing", 1);
    }
}


class ProgressRepoMock implements ilStudyProgrammeProgressRepository
{
    public $progresses = [];

    use ProgressRepoMockNIFT;

    public function read(int $id) : ilStudyProgrammeProgress
    {
        return $this->progresses[$id];
    }

    public function update(ilStudyProgrammeProgress $progress)
    {
        $this->progresses[$progress->getNodeId()] = $progress;
    }
}

class AssignmentRepoMock implements ilStudyProgrammeAssignmentRepository
{
    public $assignments = [];

    use AssignmentRepoMockNIFT;

    public function read(int $id)
    {
        return $this->assignments[$id];
    }
    public function update(ilStudyProgrammeAssignment $assignment)
    {
        $this->assignments[$assignment->getId()] = $assignment;
    }
}

class SettingsRepoMock implements ilStudyProgrammeSettingsRepository
{
    public $settings = [];

    use SettingsRepoMockNIFT;

    public function read(int $obj_id) : ilStudyProgrammeSettings
    {
        return $this->settings[$obj_id];
    }
    
    public function update(ilStudyProgrammeSettings $settings) : void
    {
        $this->settings[$settings->getObjId()] = $settings;
    }
}

class SettingsMock extends ilStudyProgrammeSettings
{
    public function __construct(int $id)
    {
        $this->obj_id = $id;
    }
}

class PrgMock extends ilObjStudyProgramme
{
    public function __construct(
        int $id,
        $env
    ) {
        $this->id = $id;
        $this->env = $env;
        $this->events = new class() {
            public function userSuccessful(ilStudyProgrammeProgress $a_progress) : void
            {
            }
        };
    }
    public function update() : void
    {
        $this->updateSettings();
    }
    protected function getLoggedInUserId() : int
    {
        return 9;
    }
    
    protected function getProgressIdString(ilStudyProgrammeProgress $progress) : string
    {
        return (string) $progress->getId();
    }

    protected function getProgressRepository() : ilStudyProgrammeProgressRepository
    {
        return $this->env->progress_repo;
    }
    protected function getAssignmentRepository() : ilStudyProgrammeAssignmentRepository
    {
        return $this->env->assignment_repo;
    }
    protected function getSettingsRepository() : ilStudyProgrammeSettingsRepository
    {
        return $this->env->settings_repo;
    }

    protected function refreshLPStatus(int $usr_id) : void
    {
    }

    public function getParentProgress(ilStudyProgrammeProgress $progress) : ?ilStudyProgrammeProgress
    {
        $parent_id = $this->env->mock_tree[$progress->getNodeId()]['parent'];
        if (is_null($parent_id)) {
            return null;
        }
        return $this->getProgressRepository()->read($parent_id);
    }

    public function getChildrenProgress($progress) : array
    {
        $progresses = [];
        foreach ($this->env->mock_tree[$progress->getNodeId()]['children'] as $child_id) {
            $progresses[] = $this->getProgressRepository()->read($child_id);
        }
        return $progresses;
    }

    public function testUpdateParentProgress(ilStudyProgrammeProgress $progress) : ilStudyProgrammeProgress
    {
        return $this->updateParentProgress($progress);
    }

    public function testApplyProgressDeadline(ilStudyProgrammeProgress $progress) : ilStudyProgrammeProgress
    {
        return $this->applyProgressDeadline($progress);
    }
}


class ilStudyProgrammeProgressCalculationsTest extends \PHPUnit\Framework\TestCase
{
    protected function buildProgramme(int $prg_id) : ilObjStudyProgramme
    {
        $prg = new PrgMock($prg_id, $this);
        $settings = new SettingsMock(
            $prg_id
        );
        $settings->setLPMode(ilStudyProgrammeSettings::MODE_POINTS);
        $set_ass = new ilStudyProgrammeAssessmentSettings(100, ilStudyProgrammeAssessmentSettings::STATUS_ACTIVE);
        $set_dl = new ilStudyProgrammeDeadlineSettings(null, null);
        $set_vq = new ilStudyProgrammeValidityOfAchievedQualificationSettings(null, null, null);
        $settings = $settings
            ->withAssessmentSettings($set_ass)
            ->withDeadlineSettings($set_dl)
            ->withValidityOfQualificationSettings($set_vq);
        $this->settings_repo->update($settings);

        $progress = (new ilStudyProgrammeProgress($prg_id * -1))
                ->withUserId(666)
                ->withStatus(ilStudyProgrammeProgress::STATUS_IN_PROGRESS)
                ->withAmountOfPoints(2)
                ->withAssignmentId(-42)
                ->withNodeId($prg_id);
        $this->progress_repo->update($progress);
        return $prg;
    }

    public function setUp() : void
    {
        $this->progress_repo = new ProgressRepoMock();
        $this->assignment_repo = new AssignmentRepoMock();
        $this->settings_repo = new SettingsRepoMock();
        $this->messages = new ilPRGMessageCollection();
        
        /*
        └── 1
            ├── 11
            │   ├── 111
            │   └── 112
            ├── 12
            └── 13
        */
        $this->mock_tree = [
            1 => ['parent' => null, 'children' => [11,12,13], 'prg' => $this->buildProgramme(1)],
                11 => ['parent' => 1, 'children' => [111,112], 'prg' => $this->buildProgramme(11)],
                    111 => ['parent' => 11, 'children' => [], 'prg' => $this->buildProgramme(111)],
                    112 => ['parent' => 11, 'children' => [], 'prg' => $this->buildProgramme(112)],
                12 => ['parent' => 1, 'children' => [], 'prg' => $this->buildProgramme(12)],
                13 => ['parent' => 1, 'children' => [], 'prg' => $this->buildProgramme(13)]
        ];

        $assignment = (new ilStudyProgrammeAssignment(-42))
            ->withRootId(1);
        $this->assignment_repo->update($assignment);
    }


    protected function getRootPrg()
    {
        return $this->mock_tree[1]['prg'];
    }

    protected function setPointsForNode(int $node_id, int $points)
    {
        $set = new ilStudyProgrammeAssessmentSettings($points, ilStudyProgrammeAssessmentSettings::STATUS_ACTIVE);
        $this->settings_repo->update($this->settings_repo->read($node_id)->withAssessmentSettings($set));
        $this->progress_repo->update($this->progress_repo->read($node_id)->withAmountOfPoints($points));

        $this->assertEquals($points, $this->mock_tree[$node_id]['prg']->getPoints());
    }

    protected function setModeForNode(int $node_id, int $mode)
    {
        $set = $this->settings_repo->read($node_id)->setLPMode($mode);
        $this->settings_repo->update($set);
    }
    
    //this is a meta-test to assure that this setup is working properly
    public function testInternalTreeIntegrity()
    {
        $progress = $this->progress_repo->read(111);
        $parent = $this->getRootPrg()->getParentProgress($progress);
        $this->assertEquals(111, $progress->getNodeId());
        $this->assertEquals(11, $parent->getNodeId());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_IN_PROGRESS, $parent->getStatus());
        $this->progress_repo->update(
            $parent->withStatus(ilStudyProgrammeProgress::STATUS_FAILED)
        );
        $progress = $this->progress_repo->read(112);
        $parent = $this->getRootPrg()->getParentProgress($progress);
        $this->assertEquals(11, $parent->getNodeId());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_FAILED, $parent->getStatus());

        $progress = $this->progress_repo->read(11);
        $this->assertEquals(
            [
                $this->progress_repo->read(111),
                $this->progress_repo->read(112)
            ],
            $this->getRootPrg()->getChildrenProgress($progress)
        );
    }

    //this is another meta-test to assure that there are no unwanted side-effects in the repos
    public function testInternalRunIntegrity()
    {
        $progress = $this->progress_repo->read(11);
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_IN_PROGRESS, $progress->getStatus());
    }


    /**
     ── 1
        ├── 11
        │   ├── 111
        │   └── 112
        ├── 12
        └── 13
     */
    public function testParentAcquisition()
    {
        $progress = $this->getRootPrg()
            ->testUpdateParentProgress($this->progress_repo->read(112));
        $this->assertEquals($this->progress_repo->read(1), $progress);
        
        $progress = $this->getRootPrg()
            ->testUpdateParentProgress($this->progress_repo->read(13));
        $this->assertEquals($this->progress_repo->read(1), $progress);
    }

    /**
     ── 1 (1)
        ├── 11 (8)
        │   ├── 111 (5)
        │   └── 112 (10) <-- turns irrelevant
        ├── 12 (12) <-- turns irrelevant
        └── 13 (10)
     */
    public function testChildrenPossiblePointsAddition()
    {
        $this->setPointsForNode(111, 5);
        $this->setPointsForNode(112, 10);
        $this->setPointsForNode(11, 8);
        $this->setPointsForNode(1, 1);
        $this->setPointsForNode(12, 12);
        $this->setPointsForNode(13, 10);

        $this->assertEquals(
            15, //111+112
            $this->getRootPrg()->getPossiblePointsOfRelevantChildren(
                $this->progress_repo->read(11)
            )
        );

        $this->assertEquals(
            30, //11+12+13
            $this->getRootPrg()->getPossiblePointsOfRelevantChildren(
                $this->progress_repo->read(1)
            )
        );

        $this->getRootPrg()->markNotRelevant(12, 6, $this->messages);
        $this->getRootPrg()->markNotRelevant(112, 6, $this->messages);
        
        $this->assertEquals(
            5, //111 (w/o + 112)
            $this->getRootPrg()->getPossiblePointsOfRelevantChildren(
                $this->progress_repo->read(11)
            )
        );

        $this->assertEquals(
            18,
            $this->getRootPrg()->getPossiblePointsOfRelevantChildren(
                $this->progress_repo->read(1)
            )
        );
    }

    /**
     ── 1 (1)
        ├── 11 (8)
        │   ├── 111 (5)
        │   └── 112 (10)
        ├── 12 (12)
        └── 13 (10)
     */
    public function testAchievedPoints()
    {
        $this->setPointsForNode(111, 5);
        $this->setPointsForNode(112, 10);
        $this->setPointsForNode(11, 8);
        $this->setPointsForNode(1, 1);
        $this->setPointsForNode(12, 12);
        $this->setPointsForNode(13, 10);
        $prg = $this->getRootPrg();

        $this->assertEquals(0, $prg->getAchievedPointsOfChildren($this->progress_repo->read(12)));
        $this->assertEquals(0, $prg->getAchievedPointsOfChildren($this->progress_repo->read(11)));
        $this->assertEquals(0, $prg->getAchievedPointsOfChildren($this->progress_repo->read(1)));

        $prg->markAccredited(12, 6, $this->messages);
        $prg->markAccredited(13, 6, $this->messages);
        $this->assertEquals(22, $prg->getAchievedPointsOfChildren($this->progress_repo->read(1)));
        $this->assertEquals(22, $this->progress_repo->read(1)->getCurrentAmountOfPoints());

        $prg->markAccredited(111, 6, $this->messages);
        $this->assertEquals(5, $prg->getAchievedPointsOfChildren($this->progress_repo->read(11)));
    }


    /**
     PRG: MODE_POINTS
     ── 1 (5)
        ├── 11 (12)
        │   ├── 111 (5)
        │   └── 112 (5)
        ├── 12 (2)
        └── 13 (2)
     */
    public function testMarkAccreditedShouldCompleteParentBySufficientPoints()
    {
        $this->setPointsForNode(111, 5);
        $this->setPointsForNode(112, 5);
        $this->setPointsForNode(11, 12);
        $this->setPointsForNode(1, 5);

        $this->getRootPrg()->markAccredited(111, 6, $this->messages);
        $this->getRootPrg()->markAccredited(112, 6, $this->messages);

        $this->assertEquals(5, $this->progress_repo->read(111)->getCurrentAmountOfPoints());
        $this->assertEquals(5, $this->progress_repo->read(112)->getCurrentAmountOfPoints());
        $this->assertEquals(10, $this->progress_repo->read(11)->getCurrentAmountOfPoints());
        $this->assertEquals(12, $this->progress_repo->read(1)->getCurrentAmountOfPoints());

        $this->assertEquals(
            16,
            $this->getRootPrg()->getPossiblePointsOfRelevantChildren(
                $this->progress_repo->read(1)
            )
        );
        $this->assertEquals(
            12,
            $this->getRootPrg()->getAchievedPointsOfChildren(
                $this->progress_repo->read(1)
            )
        );
        
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(111)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(112)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(1)->getStatus());
    }

    /**
     ── 1 (10)
        ├── 11 (5, LP)
        └── 12 (5)
     */
    public function testMarkAccreditedOnLPNode()
    {
        $this->setPointsForNode(1, 10);
        $this->setPointsForNode(11, 5);
        $this->setPointsForNode(12, 5);
        $this->setModeForNode(11, ilStudyProgrammeSettings::MODE_LP_COMPLETED);
        $this->mock_tree = [
            1 => $this->mock_tree[1],
                11 => $this->mock_tree[13],
                12 => $this->mock_tree[12]
        ];
     
        $this->getRootPrg()->markAccredited(11, 6, $this->messages);
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(5, $this->progress_repo->read(11)->getCurrentAmountOfPoints());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_IN_PROGRESS, $this->progress_repo->read(12)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_IN_PROGRESS, $this->progress_repo->read(1)->getStatus());
        
        $this->getRootPrg()->markAccredited(12, 6, $this->messages);
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(12)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(1)->getStatus());
        $this->assertEquals(5, $this->progress_repo->read(11)->getCurrentAmountOfPoints());
        $this->assertEquals(5, $this->progress_repo->read(12)->getCurrentAmountOfPoints());
        $this->assertEquals(10, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
    }

    /**
     ── 1
        ├── 11 (irrelevant)
        │   ├── 111
        │   └── 112
        ├── 12
        └── 13
     */
    public function testMarkAccreditedShouldNotChangeIrrelevantParent()
    {
        $this->progress_repo->update(
            $this->progress_repo->read(11)->withStatus(ilStudyProgrammeProgress::STATUS_NOT_RELEVANT)
        );

        $this->getRootPrg()->markAccredited(111, 6, $this->messages);
        $this->getRootPrg()->markAccredited(112, 6, $this->messages);
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(111)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(112)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_NOT_RELEVANT, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_IN_PROGRESS, $this->progress_repo->read(1)->getStatus());
        $this->assertEquals(0, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
    }

    public function testMarkRelevantShouldRecalculateStatusAndPoints()
    {
        $this->progress_repo->update(
            $this->progress_repo->read(11)->withStatus(ilStudyProgrammeProgress::STATUS_NOT_RELEVANT)
        );

        $this->getRootPrg()->markAccredited(111, 6, $this->messages);
        $this->getRootPrg()->markAccredited(112, 6, $this->messages);
        $this->assertEquals(0, $this->progress_repo->read(1)->getCurrentAmountOfPoints());

        $this->getRootPrg()->markRelevant(11, 6, $this->messages);
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(2, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
        
        $this->getRootPrg()->markNotRelevant(11, 6, $this->messages);
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_NOT_RELEVANT, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_ACCREDITED, $this->progress_repo->read(111)->getStatus());
        $this->assertEquals(0, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
    }

    /*
     ── 1 (6 points)
        ├── 11 (4 points)
        │   ├── 111 (LP-Mode, 2 points) <-- succeed
        │   └── 112 (accredited, 3 points)
        ├── 12 (2 points)
        └── 13 (accredited, 2 points)
     */
    public function testSuccessionCompletingParents()
    {
        $this->setPointsForNode(1, 6);
        $this->setPointsForNode(11, 4);
        $this->setPointsForNode(112, 3);
        $this->setModeForNode(111, ilStudyProgrammeSettings::MODE_LP_COMPLETED);

        $this->mock_tree[112]['prg']->markAccredited(112, 6, $this->messages);
        $this->mock_tree[12]['prg']->markAccredited(13, 6, $this->messages);
        $triggering_obj_id = 9001;
        $this->mock_tree[111]['prg']->succeed(111, $triggering_obj_id);

        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(111)->getStatus());
        $this->assertEquals(2, $this->progress_repo->read(111)->getCurrentAmountOfPoints());
        
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(11)->getStatus());
        $this->assertEquals(5, $this->progress_repo->read(11)->getCurrentAmountOfPoints());
        
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(1)->getStatus());
        $this->assertEquals(6, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
    }

    /*
     ── 1 (6 points)
        ├── 11 (accredited, 2 points)
        │   ├── 111
        │   └── 112
        ├── 12 (2 points)
        └── 13 (accredited, 2 points)
     */
    public function testMarkIrrelevantCompletesParent()
    {
        $this->setPointsForNode(1, 6);
        $this->getRootPrg()->markAccredited(11, 6, $this->messages);
        $this->getRootPrg()->markAccredited(13, 6, $this->messages);

        $this->assertEquals(4, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_IN_PROGRESS, $this->progress_repo->read(1)->getStatus());

        $this->getRootPrg()->markNotRelevant(12, 6, $this->messages);
        
        $this->assertEquals(4, $this->progress_repo->read(1)->getCurrentAmountOfPoints());
        $this->assertEquals(ilStudyProgrammeProgress::STATUS_COMPLETED, $this->progress_repo->read(1)->getStatus());
    }


    public function testFailByDeadline()
    {
        $past = DateTimeImmutable::createFromFormat('Ymd', '20010101');
        $progress = $this->progress_repo->read(13)->withDeadline($past);

        $progress = $this->getRootPrg()->testApplyProgressDeadline($progress);

        $this->assertEquals(
            ilStudyProgrammeProgress::STATUS_FAILED,
            $progress->getStatus()
        );
    }

    public function testDontFailByFutureDeadline()
    {
        $progress = $this->progress_repo->read(13);
        $future = (new DateTimeImmutable())->add(new DateInterval('P1D'));
        $progress = $this->getRootPrg()->testApplyProgressDeadline(
            $progress->withDeadline($future)
        );

        $this->assertEquals(
            ilStudyProgrammeProgress::STATUS_IN_PROGRESS,
            $progress->getStatus()
        );
    }

    public function testDontFailByDeadlineIfSucceeded()
    {
        $progress = $this->progress_repo->read(13)
            ->withStatus(ilStudyProgrammeProgress::STATUS_COMPLETED);
        $this->progress_repo->update($progress);

        $past = DateTimeImmutable::createFromFormat('Ymd', '20010101');
        $progress = $this->getRootPrg()->testApplyProgressDeadline(
            $progress->withDeadline($past)
        );

        $this->assertEquals(
            ilStudyProgrammeProgress::STATUS_COMPLETED,
            $progress->getStatus()
        );
    }

    public function testChangingDeadline()
    {
        $past = DateTimeImmutable::createFromFormat('Ymd', '20010101');
        $this->getRootPrg()->changeProgressDeadline(12, 6, $this->messages, $past);
        
        $this->assertEquals(
            ilStudyProgrammeProgress::STATUS_FAILED,
            $this->progress_repo->read(12)->getStatus()
        );
    }

    public function testDontChangeDeadlineForCompleted()
    {
        $this->progress_repo->update(
            $this->progress_repo->read(12)
            ->withStatus(ilStudyProgrammeProgress::STATUS_COMPLETED)
        );

        $past = DateTimeImmutable::createFromFormat('Ymd', '20010101');
        $this->getRootPrg()->changeProgressDeadline(12, 6, $this->messages, $past);
        
        $progress = $this->progress_repo->read(12);
        $this->assertEquals($progress->getStatus(), ilStudyProgrammeProgress::STATUS_COMPLETED);
        $this->assertNull($progress->getDeadline());
    }

    public function testTranstitionToProgressWithPastDeadline()
    {
        $past = DateTimeImmutable::createFromFormat('Ymd', '20010101');
        $this->progress_repo->update(
            $this->progress_repo->read(11)
                ->withStatus(ilStudyProgrammeProgress::STATUS_ACCREDITED)
                ->withDeadline($past)
        );

        $this->getRootPrg()->unmarkAccredited(11, 6, $this->messages);
        
        $this->assertEquals(
            ilStudyProgrammeProgress::STATUS_FAILED,
            $this->progress_repo->read(11)->getStatus()
        );
    }

    public function testMarkAsIndividual()
    {
        $this->assertFalse($this->progress_repo->read(11)->hasIndividualModifications());
        
        $this->getRootPrg()->markNotRelevant(11, 6, $this->messages);
        $this->assertTrue($this->progress_repo->read(11)->hasIndividualModifications());

        $future = (new DateTimeImmutable())->add(new DateInterval('P1D'));
        
        $this->getRootPrg()->changeProgressDeadline(12, 6, $this->messages, $future);
        $this->assertTrue($this->progress_repo->read(12)->hasIndividualModifications());
        
        $this->getRootPrg()->changeProgressValidityDate(13, 6, $this->messages, $future);
        $this->assertTrue($this->progress_repo->read(13)->hasIndividualModifications());
    }

    public function testUpdateFromSettingsResetsIndividual()
    {
        $future = (new DateTimeImmutable())->add(new DateInterval('P1D'));
        $this->getRootPrg()->changeProgressDeadline(12, 6, $this->messages, $future);
        $this->assertTrue($this->progress_repo->read(12)->hasIndividualModifications());
        $this->getRootPrg()->updateProgressFromSettings(12, 6);
        $this->assertFalse($this->progress_repo->read(12)->hasIndividualModifications());
    }

    public function testUpdateFromSettings()
    {
        $future = (new DateTime())->add(new DateInterval('P1D'));
        $future2 = (new DateTime())->add(new DateInterval('P4D'));
        $points = 73;

        $set_dl = new ilStudyProgrammeDeadlineSettings(null, $future);
        $set_vq = new ilStudyProgrammeValidityOfAchievedQualificationSettings(null, $future2, null);
        $set_as = new ilStudyProgrammeAssessmentSettings($points, ilStudyProgrammeAssessmentSettings::STATUS_ACTIVE);
        $settings = $this->mock_tree[111]['prg']->getSettings()
            ->withDeadlineSettings($set_dl)
            ->withValidityOfQualificationSettings($set_vq)
            ->withAssessmentSettings($set_as);

        $this->mock_tree[111]['prg']->markAccredited(111, 6, $this->messages);
        $this->mock_tree[111]['prg']->updateSettings($settings);

        $this->mock_tree[111]['prg']->updateProgressFromSettings(111, 6);
        $progress = $this->progress_repo->read(111);

        $this->assertEquals($future, $progress->getDeadline());
        $this->assertEquals($future2, $progress->getValidityOfQualification());
        $this->assertEquals($points, $progress->getAmountOfPoints());
    }
}
